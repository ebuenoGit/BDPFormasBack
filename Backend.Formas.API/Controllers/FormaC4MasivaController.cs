...
namespace Backend.Formas.API.Controllers
{
    ...
    /// MCG  --- > Mejoras BDP ++ 
    /// 2022 - Mar - 11
    /// Carga de archivo Excel Formato predefindo por ecopetrol del Cuadro 4 que incluye el pden_id PDEN_LAND_RIGHT 
    /// 
    /// </summary>
    [Route("api/[controller]")]
    [ApiController]
    public class FormaC4MasivaController : Base.BaseController
    {
        /// <summary>
        /// The Business 
        /// </summary>
        private readonly IFormasC4MasivaBusiness _business;

        /// <summary>
        /// Initializes a new instance of the <see cref="FormaC4MasivaController"/> class.
        /// </summary>
        /// <param name="business"></param>
        public FormaC4MasivaController(IFormasC4MasivaBusiness business)
        {
            _business = business;
        }
        /// <summary>
        ...
        /// <returns></returns>
        [HttpPost("actualizaC4")]
        public async Task<ActionResult> CreateForm(DateTime date, RequestCreateFormaC4Masiva data)
        {
            int nCodes = 0;
            string cRespuestas = "";
            try
            {
                //registerForm 
                //Detail9Multiple
                var result = await _business.GuardarFormaC4PPDM(data.DetaSaveBDP);
                var resultMuni = await _business.GuardarFormaC4PPDMuni(data.DetaSaveMuni);
        ...
        /// <summary>
        /// consultaFormC4Masiva 
        /// </summary>
        /// <param name="nAnio"></param>
        /// <param name="nMes"></param>
        /// <param name="nTodo"></param>
        /// <returns></returns>
        [HttpGet("consultaFormC4Masiva")]
        public async Task<ActionResult> ConsultarC4(int nAnio, int nMes, int nTodo)
        {
            try
            {
                var result = await _business.consultaformaC4(nAnio, nMes, nTodo);
                return StatusCode(200, result);
            }
            catch (Exception e)
            {
        ...
        /// <param name="nTodo"></param>
        /// <returns></returns>
        [HttpGet("consultaFormC4Muni")]
        public async Task<ActionResult> ConsultarC4Muni(int nAnio, int nMes, int nTodo)
        {
            try
            {
                var result = await _business.consultaformaC4Muni(nAnio, nMes, nTodo);
                return StatusCode(200, result);
            }
            catch (Exception e)
            {
